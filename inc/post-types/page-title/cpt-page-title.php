<?php
namespace Shadhinplugins\CPT\PageTitle;
 ...
 * Singleton class
 * class CPT_PageTitle
 * @package Shadhinplugins\CPT\PageTitle;
 */
final class CPT_PageTitle implements Lib\Shadhin_plugins_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Page Title', 'shadhin-plugins' );
		$this->ptPluralName = esc_html__( 'Parts - Page Title', 'shadhin-plugins' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-page-title'	=> esc_html__( 'Currently Active Page Title', 'shadhin-plugins' ),
		...
		switch( $columns ) {
			case 'active-page-title' :
				if( shadhin_plugins_theme_installed() ) {
					$active_headertop_id = shadhin_get_redux_option( 'page-title-settings-choose-page-title-cpt-widget-area', 'default' );
