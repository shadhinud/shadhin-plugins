<?php
namespace Shadhinplugins\CPT\MegaMenu;
 ...
 * Singleton class
 * class CPT_MegaMenu
 * @package Shadhinplugins\CPT\MegaMenu;
 */
final class CPT_MegaMenu implements Lib\Shadhin_plugins_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Mega Menu', 'shadhin-plugins' );
		$this->ptPluralName = esc_html__( 'Parts - Mega Menu', 'shadhin-plugins' );
		$this->ptKey = 'megamenu';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-megamenu'	=> esc_html__( 'Currently Active Mega Menu', 'shadhin-plugins' ),
		...
		switch( $columns ) {
			case 'active-megamenu' :
				if( shadhin_plugins_theme_installed() ) {
					$active_headertop_id = shadhin_get_redux_option( 'megamenu-settings-choose-megamenu-cpt-widget-area', 'default' );
