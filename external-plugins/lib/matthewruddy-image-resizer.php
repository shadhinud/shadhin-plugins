...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function shadhin_plugins_matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {

		global $wpdb;

		if ( empty( $url ) ) {
			//return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.','shadhin-plugins' ), $url );
			$image_array = array();
		...
		$name = wp_basename( $file_path, ".$ext" );
			...
			if ( 'bmp' == $ext ) {
			return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.','shadhin-plugins' ), $url );
		}
 ...
 */
if (!function_exists('shadhin_plugins_matthewruddy_delete_resized_images')) {
	add_action( 'delete_attachment', 'shadhin_plugins_matthewruddy_delete_resized_images' );
	function shadhin_plugins_matthewruddy_delete_resized_images( $post_id ) {
